...
Configuration is private in `%LOCALAPPDATA%\CUA-LAB\provider.json`; it is excluded from the bundle. Models cannot be switched while a task or another model operation is active. Local server providers accept only loopback HTTP addresses, send no OpenRouter credentials, and never fall back to a cloud provider. Ollama cloud models are rejected. Screenshot input requires a vision-capable model; text models can work with UI Automation. The built in GGUF engine is bundled with the EXE, while model weight files remain outside the bundle.
