...
try {
    ...
    $Process = Start-Process -FilePath '.\dist\CUA-LAB\CUA-LAB.exe' -ArgumentList '--smoke-test' -PassThru
    if (-not $Process.WaitForExit(45000)) { $Process.Kill(); throw 'Packaged startup smoke test timed out.' }
    if ($Process.ExitCode -ne 0) { throw 'Packaged startup smoke test failed.' }
